...
export class UserInformationsService extends BaseServiceService {
  ...
  constructor(private http: HttpClient, protected centralNotifyService: CentralNotifyService) {   
    ...
    this.centralNotifyService.notifyCreatePosition.subscribe((userPositionResponse) => {
      if (userPositionResponse != null) {
        if (this.userInformations != null) {
          this.userInformations.accountAmount -= userPositionResponse.amount * userPositionResponse.price
        }
      }
    });
  }
